module Murocast.Shared.DateTime
open System
...
module DateRange =
    ...
    let dateRangeToDays (startDate:DateTimeOffset,endDate:DateTimeOffset) =
        List.map ((fun x -> startDate.AddDays (float x)) >> (fun x -> DateTimeOffset(DateTime(x.Year, x.Month, x.Day, 0, 0, 0)))) (endDate.Subtract(startDate).TotalDays
        |> int
        |> (fun d -> [0..d]))
